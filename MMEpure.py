...
parser.add_argument('--save_path', type=str, default='./save_april/MMEpure/exp1',
                    help='dir to save model')

parser.add_argument('--steps', type=int, default=200000, metavar='N',
...
def train():
    ...
    for step in range(start_step, all_step):
        ...
        # unlcat = torch.cat((im_data_tu, im_data_tu2), 0)
        unlcat = im_data_tu
        unlabel_raw_output = G(unlcat)
        ...
        if step % args.log_interval == 0 and step > 4000:
            log_train = 'Ep: {} lr: {}, loss_all: {:.6f}, loss_c: {:.6f}, loss_mme: {:.6f}'.format(step, lr, \
                    loss_comb, loss_c, -loss_t)
            print(log_train)
            with open(record_train_file,'a') as f:
                f.write(log_train+'\n')
