...
parser.add_argument('--cudanum', type=str, default='3',help='resume from checkpoint')
parser.add_argument('--ways', type=int, default=20, help='number of classes sampled')
...
parser.add_argument('--save_path', type=str, default='./save_may/finetune1/exp2',
...
def train():
    ...
    for step in range(start_step, all_step):
        ...
        loss_c = criterion(out1[:nt], target)     
        ...
        # pseudo_label = torch.softmax(out1[nt:nt+nu+nu].detach(), dim=-1)
        # max_probs, targets_u = torch.max(pseudo_label, dim=-1)
        # mask = max_probs.ge(args.threshold).float()
        ...
        #     loss_u = criterion_un(out1[nt+nu:], targets_u, mask)
        # else:            
        #     loss_u = (F.cross_entropy(out1[nt:], targets_u, reduction='none') * mask).mean()
        ...
        if step % args.log_interval == 0:
            ...
            # log_train = 'Ep: {} lr: {}, loss_all: {:.6f}, loss_c: {:.6f}'.format(step, lr, \
            #         loss_comb, loss_c)#, loss_u, -loss_t)
            print(log_train)
            with open(record_train_file,'a') as f:
                f.write(log_train+'\n')
