...
class PrototypeLoss(object):
    ...
    def __call__(self,gt_labels_s, src_feat, target_feat, normalize_feature=False):
        ...
        # breakpoint()
        print(proto.size()) #6*512
        if self.trg_shots > 1:
            proto = proto.reshape(self.trg_shots, self.ways, -1)
            # print(proto.size())
            proto=proto.mean(dim=0) #2,3,512
            # print(proto.size())
            
        n = src_feat.shape[0]
        m = proto.shape[0]
        print(src_feat.size(),proto.size())
        src_feat = src_feat.unsqueeze(1).expand(n, m, -1) # 12*512
        proto = proto.unsqueeze(0).expand(n, m, -1)       # 3*512
        logits = -((src_feat - proto)**2).sum(dim=2)  # batchsize * shot类别数量 12*3

        # print("gt_labels_s:",gt_labels_s)
        # gt_labels_s=gt_labels_s.reshape(self.ways, self.src_shots).transpose(1,0)# 4,3->3
        # print("gt_labels_s:",gt_labels_s,gt_labels_s[0])
        # loss = F.cross_entropy(logits, self.label)
        loss = F.cross_entropy(logits, gt_labels_s) # label= 0....类别数量-1 
        #label长度为12,内容包括0 1 2
        #我呢？
        # logits=24*126 
...
class PrototypeLoss2(object):
    def __init__(self,ways=10, trg_shots=3, src_shots=10):
        a=1
        self.a=a
        #什么用都没

  #  (args.protow, device, step, ns, nt, nu, prototype, output, target, max_probs, mask, targets_u)
    def __call__(self,protow,device,step,bs_s,bs_t,bs_u,prototype,feature,label,max_probs,mask, targets_u):
        src_feat = feature[:bs_s]
        target_feat = feature[bs_s:bs_s+bs_t]
        unlabeled_feat = feature[bs_s+bs_t:bs_s+bs_t+bs_u]
        source_label=label[:bs_s]
        target_label=label[bs_s:bs_s+bs_t]
        src_feat = normalize(src_feat, axis=-1)
        target_feat = normalize(target_feat, axis=-1)
        unlabeled_feat = normalize(unlabeled_feat, axis=-1)
        class_num=prototype.size(0)
        inc=prototype.size(1)
        prototype = prototype.to(device)
        unlabeled_feat = unlabeled_feat.to(device)
        target_feat = target_feat.to(device)

        #先看y1==y2 知道是否reliable
        y1 = targets_u #通过argmax(pseudo label)得到伪标签y1
        softmax=nn.Softmax(dim=-1)
        # unlabeled_feat 24* 512 -> 24*126*512
        # prototype 126*512      -> 24*126*512
        part=-((unlabeled_feat.unsqueeze(1).expand(bs_u,class_num,-1)-prototype.unsqueeze(0).expand(bs_u,class_num,-1))**2).sum(dim = 2)
        dist = normalize(softmax(part))
        _ , y2 = torch.max(dist, dim = -1)
        result = (y2.eq(y1)) *mask
        if step%1000==0:
            print("result:",y2.eq(y1))
#########生成target的prototype######################################
        proto_target = torch.FloatTensor(1).to(device)
        proto_target = Variable(proto_target)
        proto_target = proto_target.resize_(class_num,inc).copy_(torch.zeros(class_num,inc))
        for y in target_label:
            mask_t= target_label==y
            if torch.any(mask_t):
                proto_target[y]=torch.mean(target_feat[mask_t],dim=0)
#########生成unlabel的prototype########################################    
        proto_unlabel = torch.FloatTensor(1).to(device)
        proto_unlabel = Variable(proto_unlabel)
        proto_unlabel = proto_unlabel.resize_(class_num,inc).copy_(torch.zeros(class_num,inc))
        unlabeled_feat = unlabeled_feat * (mask.unsqueeze(1).expand(bs_u,inc))
        sig=nn.Sigmoid()
        for y in targets_u:
            mask_u= targets_u==y
            if torch.any(mask_u):
                prob=sig(max_probs.unsqueeze(1).expand(bs_u,inc))
                unlabeled_feat_= unlabeled_feat*prob
                proto_unlabel[y]=torch.mean(unlabeled_feat_[mask_u],dim=0) 
        proto=proto_target+proto_unlabel #生成bs的target+unlabel的prototype
        prototype_nw=prototype*0.9+proto*0.1 #更新prototype
        prototype_new=prototype_nw.detach()
#########计算source的loss########################################
        loss=0.0
        label=torch.arange(class_num).to(device)
        src_feat=src_feat.unsqueeze(1).expand(bs_s, class_num, -1) # 24,512
        proto_=prototype_new.unsqueeze(0).expand(bs_s, class_num, -1) #126,512
        logits = -((src_feat - proto_)**2).sum(dim=2) # 24*126
        loss = F.cross_entropy(logits, source_label)
        return loss, prototype_new
