...
#:ns,ns:ns+nt==source,target
    def __call__(self, src_feat, target_feat, normalize_feature=False):

        if normalize:
            src_feat = normalize(src_feat, axis=-1)#最后一维做正则化
            proto = normalize(target_feat, axis=-1)

        # breakpoint()

        if self.trg_shots > 1:
            proto = proto.reshape(self.trg_shots, self.ways, -1).mean(dim=0) #3，10


        n = src_feat.shape[0] #24
        m = proto.shape[0] #10
