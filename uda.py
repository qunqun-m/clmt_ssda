...
parser.add_argument('--save_path', type=str, default='./save_april/exp4',
...
parser.add_argument('--resume', action='store_true', default='/home/chuqs/code/jmq/pacl/pacl_lk/pacl/save_april/exp4/save_model/temp.log_real_to_sketch_step_88000_resnet34.pth',
#'/home/chuqs/code/jmq/pacl/pacl_lk/pacl/save_april/exp4/save_model/temp.log_real_to_sketch_step_28000_resnet34.pth',
...
def train():
    ...
    for step in range(start_step, all_step):
        ...
        data = torch.cat((im_data_s,im_data_tu, im_data_tu2), 0)
        target=gt_labels_s
        ...
        loss_c = criterion(out1[:ns], target)     

        pseudo_label = torch.softmax(out1[ns:ns+nu].detach(), dim=-1)
        max_probs, targets_u = torch.max(pseudo_label, dim=-1)
        mask = max_probs.ge(args.threshold).float()

        if args.kld:
            loss_u = criterion_un(out1[ns+nu:], targets_u, mask)
        else:            
            loss_u = (F.cross_entropy(out1[ns+nu:], targets_u, reduction='none') * mask).mean()


        # if args.alpha == 0:
        #     proto_loss = 0
        # else:
        #     proto_loss,nwp = criterion_aux(device,step,ns,nt,nu,prototype,output,target,max_probs,mask, targets_u)
        #     prototype.copy_(nwp)
        ...
        if step % args.log_interval == 0 and step > 0:
            ...
            #         loss_comb, loss_c, proto_loss, loss_u, -loss_t)
            # print(log_train)
        ...
        if step % args.save_interval == 0 and step > 10000:
            print('saving model')
